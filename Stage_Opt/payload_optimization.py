...
def generate_report(parameters, stages, results):
    ...
    latex_doc = [
        ...
        "\\small",  # Slightly reduce font size for better fit
        results_df.to_latex(
            index=False,
            escape=True,
            float_format=lambda x: '{:.6f}'.format(x) if abs(x) < 1000 else '{:.2e}'.format(x),
            column_format='l' + 'c' * (len(results_df.columns) - 1),  # Left align first column, center others
            na_rep='--'
    ...
    # Extract stage ratios if available, otherwise use full results
    if "Stage Ratios" in results_df.columns:
        stage_ratios_df = results_df[["Solver", "Stage Ratios"]]
        latex_doc.extend([
            "\\begin{table}[H]",
            "\\centering",
            "\\caption{Stage Ratios by Solver}",
            stage_ratios_df.to_latex(
                index=False,
                escape=True,
                column_format='lc',
                na_rep='--'
